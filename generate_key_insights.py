#!/usr/bin/env python3
"""
Generate Key Insights for industry_heatmap.html
...
def extract_data_from_html(content):
    """Extract the data object from the HTML JavaScript"""
...
def extract_ranklists_from_html(content):
    ...
    # Parse ranklists - this is complex, so we'll use a simpler approach
...
def generate_main_insights_html(insights):
    """Generate HTML for main heatmap Key Insights"""
...
'''
    ...
    if insights['contraction_to_growth']:
        ...
        html += f'''{industries_str} have shifted from negative to positive territory, signaling renewed demand in these sectors.</p>
        '''
    else:
        html += '''None in the latest report.</p>
        '''
    
    html += '''<p><strong>Industries Moving from Growth to Contraction (Latest Report):</strong> '''
...
def generate_new_orders_insights_html(insights):
    """Generate HTML for New Orders Key Insights"""
    html = '''    <div class="summary-box">
...
'''
    ...
    if insights['contraction_to_growth']:
        industries_str = ', '.join([f'<strong>{ind}</strong>' for ind in insights['contraction_to_growth']])
        html += f'''{industries_str} have all shifted to positive new order growth, indicating renewed demand.</p>
        '''
    else:
        html += '''None in the latest report.</p>
        '''
    ...
    if insights['growth_to_contraction']:
        industries_str = ', '.join([f'<strong>{ind}</strong>' for ind in insights['growth_to_contraction']])
        html += f'''{industries_str} have reversed from growth to contraction.</p>
    </div>'''
    else:
        html += '''None in the latest report.</p>
    </div>'''
...
def update_key_insights():
    ...
    print("Reading industry_heatmap.html...")
    ...
    # Generate new HTML
    print("Generating new Key Insights HTML...")
    main_insights_html = generate_main_insights_html(main_insights)
    new_orders_insights_html = generate_new_orders_insights_html(new_orders_insights)
    
    # Replace in content
    print("Updating HTML file...")
    
    # Replace first Key Insights (main heatmap)
    # Find the summary-box after the first table and before "New Orders"
    pattern1 = r'(<table id="heatmapTable">.*?</table>\s*)<div class="summary-box">.*?</div>'
    replacement1 = r'\1' + main_insights_html
    content = re.sub(pattern1, replacement1, content, count=1, flags=re.DOTALL)
    
    # Replace second Key Insights (New Orders)
    # Find the summary-box after the rankTable and before "ISM Manufacturing PMI Data"
    pattern2 = r'(<table id="rankTable">.*?</table>\s*)<div class="summary-box">.*?</div>'
    replacement2 = r'\1' + new_orders_insights_html
    ...
    print("✓ Key Insights updated successfully!")
    return True
